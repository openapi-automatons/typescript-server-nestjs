...
/**
 * Emit services/<tag>.service.ts: an abstract class declaring the contract the user must implement.
...
export const emitService = (api: Api): string =>
  render((sf) => {
    ...
    const serviceClass = sf.addClass({isExported: true, isAbstract: true, name: `${baseName(api)}Service`});
